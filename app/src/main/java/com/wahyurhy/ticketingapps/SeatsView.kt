package com.wahyurhy.ticketingapps
...
class SeatsView : View {
    ...
    private fun drawSeat(canvas: Canvas?, seat: Seat) {
        ...
        numberSeatPaint.apply {
            textSize = 50F
            numberSeatPaint.getTextBounds(seat.name, 0, seat.name.length, mmBounds)
        }
        canvas?.drawText(seat.name, 100F - mmBounds.centerX(), 100F, numberSeatPaint)
